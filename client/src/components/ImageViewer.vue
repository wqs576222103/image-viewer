<template>
  <div class="container">
    <div class="header">
      ...
      <el-button type="primary" @click="showAddForm = true">Add Image</el-button>
    </div>
    ...
    <el-dialog :title="editMode ? 'Edit Image' : 'Add Image'" v-model="showAddForm" width="500px">
      <el-form :model="imageForm" ref="imageFormRef" label-width="100px">
        <el-form-item label="Image" prop="image">
          <input type="file" @change="handleFileUpload" accept="image/*" />
          <div v-if="imagePreview">
            <img :src="imagePreview" style="width: 100px; height: 100px; object-fit: cover;" />
          </div>
        </el-form-item>
        <el-form-item label="Category" prop="category">
          <el-input v-model="imageForm.category" placeholder="Enter category"></el-input>
        </el-form-item>
        <el-form-item label="Remark" prop="remark">
          <el-input v-model="imageForm.remark" type="textarea" placeholder="Enter remark"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
        <span class="dialog-footer">
          <el-button @click="showAddForm = false">Cancel</el-button>
          <el-button type="primary" @click="saveImage">{{ editMode ? 'Update' : 'Add' }}</el-button>
        </span>
      </template>
    </el-dialog>
    ...
    <div class="image-grid">
      <div class="image-card" v-for="image in images" :key="image.id">
        <img :src="image.imagePath" :alt="image.remark" class="image-preview" />
        <div class="image-info">
          <p><strong>Category:</strong> {{ image.category || '' }}</p>
          <p><strong>Remark:</strong> {{ image.remark || '' }}</p>
          <p><strong>updatedAt:</strong> {{ image.updatedAt || '' }}</p>
        </div>
        <div class="actions">
          <el-button size="small" @click="editImage(image)">Edit</el-button>
          <el-button size="small" type="danger" @click="deleteImage(image.id)">Delete</el-button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  setup() {
    ...
    const deleteImage = async (id) => {
      try {
        await ElMessageBox.confirm('Are you sure you want to delete this image?', 'Warning', {
    ...
    const searchImages = async () => {
      try {
        ...
        images.value = (response.data || []).map(v => {
          return {
            ...v,
